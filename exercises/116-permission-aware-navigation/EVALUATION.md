# Avaliação: Navegação orientada por permissões
    ...
    Avaliar navegação, delimitação de responsabilidade e implementação de autorização de interface. O caminho da fachada até a política atravessa limites explícitos da aplicação; alterar arquivos não relacionados deve ser evitado.
    ...
    **Armadilha:** criar uma segunda implementação fora do fluxo real, alterar a fachada para contornar a arquitetura ou confundir módulos plausíveis com arquivos que precisam ser modificados.
    ...
    | --- | --- | ---: | --- | --- |
    | C01 | minimum | 3 | mostra rota permitida | `C01` |
| C02 | minimum | 2 | oculta rota negada | `C02` |
| C03 | minimum | 2 | múltiplas permissões | `C03` |
| C04 | intermediate | 2 | rota pública permanece | `C04` |
