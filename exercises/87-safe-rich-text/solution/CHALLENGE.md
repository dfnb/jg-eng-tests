# Desafio: Texto rico seguro

    ## Contexto

    Escapar HTML não confiável sem remover texto Unicode.

    ## Pedido

    Complete `evaluate` preservando sua exportação e integre a decisão ao componente quando isso for relevante. Considere os limites naturais de **segurança XSS**, não somente o caminho mais comum.
