# Desafio: Validação assíncrona de campo

    ## Contexto

    Aceitar resultado somente da geração mais recente.

    ## Pedido

    Complete `evaluate` preservando sua exportação e integre a decisão ao componente quando isso for relevante. Considere os limites naturais de **forms**, não somente o caminho mais comum.
