# Avaliação: Fallback de área de transferência
    ...
    Avaliar navegação, delimitação de responsabilidade e implementação de integração com navegador. O caminho da fachada até a política atravessa limites explícitos da aplicação; alterar arquivos não relacionados deve ser evitado.
    ...
    **Armadilha:** criar uma segunda implementação fora do fluxo real, alterar a fachada para contornar a arquitetura ou confundir módulos plausíveis com arquivos que precisam ser modificados.
    ...
    | --- | --- | ---: | --- | --- |
    | C01 | minimum | 3 | API moderna em contexto seguro | `C01` |
| C02 | minimum | 2 | contexto inseguro usa fallback | `C02` |
| C03 | minimum | 2 | API ausente usa fallback | `C03` |
| C04 | intermediate | 2 | permissão negada orienta manual | `C04` |
| C05 | desired | 1 | prompt ainda tenta API | `C05` |
