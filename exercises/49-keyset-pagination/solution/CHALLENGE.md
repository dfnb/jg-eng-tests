# Desafio: Paginação keyset

    ## Contexto

    Decidir se registro vem depois do cursor composto `(score desc, id asc)`.

    ## Pedido

    Complete `Policy.Evaluate` de acordo com o contrato descrito pelo domínio e preserve a assinatura pública. Investigue os casos de borda coerentes com **queries** e acrescente testes de regressão quando necessário.
