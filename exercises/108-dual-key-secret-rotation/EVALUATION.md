# Avaliação: Rotação de segredo sem indisponibilidade
    ...
    Avaliar navegação, delimitação de responsabilidade e implementação de gestão de segredos. O caminho da fachada até a política atravessa limites explícitos da aplicação; alterar arquivos não relacionados deve ser evitado.
    ...
    **Armadilha:** criar uma segunda implementação fora do fluxo real, alterar a fachada para contornar a arquitetura ou confundir módulos plausíveis com arquivos que precisam ser modificados.
    ...
    | --- | --- | ---: | --- | --- |
    | C01 | minimum | 3 | chave atual vale | `C01` |
| C02 | minimum | 2 | chave anterior vale na janela | `C02` |
| C03 | minimum | 2 | chave antiga demais falha | `C03` |
| C04 | intermediate | 2 | anterior vazia não aceita vazio | `C04` |
| C05 | desired | 1 | comparação é exata | `C05` |
