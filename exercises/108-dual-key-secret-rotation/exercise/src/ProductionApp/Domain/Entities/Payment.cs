    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public Payment Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public Payment ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public Payment ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static Payment Create(string id, string tenantId, DateTimeOffset now) =>
