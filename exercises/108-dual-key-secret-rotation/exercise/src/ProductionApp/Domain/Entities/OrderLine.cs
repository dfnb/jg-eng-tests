    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public OrderLine Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public OrderLine ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public OrderLine ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static OrderLine Create(string id, string tenantId, DateTimeOffset now) =>
