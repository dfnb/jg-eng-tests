# Avaliação: Estorno no razão contábil
    ...
    Avaliar navegação, delimitação de responsabilidade e implementação de invariantes contábeis. O caminho da fachada até a política atravessa limites explícitos da aplicação; alterar arquivos não relacionados deve ser evitado.
    ...
    **Armadilha:** criar uma segunda implementação fora do fluxo real, alterar a fachada para contornar a arquitetura ou confundir módulos plausíveis com arquivos que precisam ser modificados.
    ...
    | --- | --- | ---: | --- | --- |
    | C01 | minimum | 3 | débito vira crédito | `C01` |
| C02 | minimum | 2 | crédito vira débito | `C02` |
| C03 | minimum | 2 | centavos são preservados | `C03` |
| C04 | intermediate | 2 | valor não muda de sinal | `C04` |
| C05 | desired | 1 | moeda é normalizada | `C05` |
