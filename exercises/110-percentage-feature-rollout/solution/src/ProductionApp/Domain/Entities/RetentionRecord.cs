    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public RetentionRecord Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public RetentionRecord ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public RetentionRecord ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static RetentionRecord Create(string id, string tenantId, DateTimeOffset now) =>
