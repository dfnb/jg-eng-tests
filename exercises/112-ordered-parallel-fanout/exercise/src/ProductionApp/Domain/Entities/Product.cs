    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public Product Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public Product ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public Product ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static Product Create(string id, string tenantId, DateTimeOffset now) =>
