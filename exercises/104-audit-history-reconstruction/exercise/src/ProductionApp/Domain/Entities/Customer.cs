    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public Customer Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public Customer ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public Customer ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static Customer Create(string id, string tenantId, DateTimeOffset now) =>
