# Avaliação: Precedência de configuração regional
    ...
    Avaliar navegação, delimitação de responsabilidade e implementação de configuração. O caminho da fachada até a política atravessa limites explícitos da aplicação; alterar arquivos não relacionados deve ser evitado.
    ...
    **Armadilha:** criar uma segunda implementação fora do fluxo real, alterar a fachada para contornar a arquitetura ou confundir módulos plausíveis com arquivos que precisam ser modificados.
    ...
    | --- | --- | ---: | --- | --- |
    | C01 | minimum | 3 | tenant vence | `C01` |
| C02 | minimum | 2 | regional cobre tenant vazio | `C02` |
| C03 | minimum | 2 | global é fallback | `C03` |
| C04 | intermediate | 2 | espaço não sobrescreve | `C04` |
| C05 | desired | 1 | tudo ausente é unset | `C05` |
