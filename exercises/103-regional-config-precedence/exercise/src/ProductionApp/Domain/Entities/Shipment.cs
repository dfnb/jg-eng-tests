    ...
    DateTimeOffset UpdatedAt)
{
    ...
    public Shipment Touch(DateTimeOffset now) => this with { UpdatedAt = now };
    public Shipment ChangeStatus(string status, DateTimeOffset now) => this with { Status = status.Trim().ToLowerInvariant(), UpdatedAt = now };
    public Shipment ChangeAmount(decimal amount, DateTimeOffset now) => this with { Amount = amount, UpdatedAt = now };
    ...
    public static Shipment Create(string id, string tenantId, DateTimeOffset now) =>
